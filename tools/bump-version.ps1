<#
.SYNOPSIS
Sets the package version everywhere it lives, so no release ships with the copies disagreeing.

.DESCRIPTION
One version number lives in the csproj <Version>, the umbraco-package.json "version", and a ?v= cache-buster on
every asset path in that manifest. The build workflow fails when they drift, which makes a manual bump a game of
find-them-all. This is the find-them-all.
...
if ($updatedCsproj -notmatch [regex]::Escape("<Version>$Version</Version>")) {
